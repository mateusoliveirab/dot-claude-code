...
- [ ] Step 1: Load credentials (LOW freedom)
- [ ] Step 2: Validate credentials (LOW freedom)
- [ ] Step 3: Initiate crawl (LOW freedom)
- [ ] Step 4: Poll for completion (LOW freedom)
- [ ] Step 5: Retrieve results (MED freedom)
- [ ] Step 6: Save results (MED freedom)
...
**Validation**: If either is missing, stop and show the setup instructions from Step 1. Do not proceed.
...
When `--since` is provided, convert to Unix timestamp: `date -d "YYYY-MM-DD" +%s` (Linux) or `date -j -f "%Y-%m-%d" "YYYY-MM-DD" +%s` (macOS).
...
```
/cf-crawl https://docs.example.com --limit 50 --since YYYY-MM-DD
```
...
- `--since DATE`: only crawl pages modified since DATE (ISO date like `YYYY-MM-DD` or Unix timestamp). Converts to Unix timestamp for the `modifiedSince` API parameter
...
- [Full API parameter reference](references/cloudflare-api.md)
