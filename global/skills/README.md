...
| `task-splitter` | Split tasks across multiple AI CLI tools (opencode, gemini, codex, ollama) | `/task-splitter` |
...
| `verify-dot-claude-code-repo` | Run full validation checklist for this repo | `/verify-dot-claude-code-repo` |
...
- `description` (required): When to use, max 1024 chars — include keywords for matching

> Note: Fields such as `auto_invoke`, `compatibility`, `trigger`, `keywords`, and `tags` are **not valid** — the audit script flags them as errors.
...
python global/skills/skill-creator/scripts/quick_validate.py global/skills/<skill-name>
```
